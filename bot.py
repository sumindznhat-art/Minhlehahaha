...
BOT_TOKEN = os.environ["8862072402:AAG2T5KXVsaqSQPsQ25sj-HkClBExDVz7Jk"]   # ❗ KHÔNG hardcode token

# Lưu state tạm theo user_id
    ...
    try:
        ...
        if state:
            if state["step"] == "md5":
                ...
                state["step"] = "hash"
                await update.message.reply_text(
                    "🔹 Bước 2/2: Gửi **Hash** (64 ký tự hex).",
                    parse_mode="Markdown",
                )
                return
            if state["step"] == "hash":
                ...
                await update.message.reply_text(
                    _format_result(result), parse_mode="Markdown"
                )
                return
